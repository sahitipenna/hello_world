...
export default function OnboardingModal({
  ...
  async function save() {
    setSaving(true);
    try {
      await onSave(Array.from(selected), timeBudget);
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-paper rounded-2xl shadow-2xl max-w-md w-full p-6 sm:p-7 animate-fade-in max-h-[90vh] overflow-y-auto">
        <button
          onClick={onClose}
          aria-label="Close"
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {TIME_BUDGETS.map((mins) => {
            const active = timeBudget === mins;
            return (
              <button
                key={mins}
                onClick={() => setTimeBudget(active ? null : mins)}
        ...
        <div className="flex items-center gap-3">
          ...
          >
            {selected.size === 0 && timeBudget === null ? "Skip for now" : "Save & continue"}
          </button>
        </div>
      </div>
    </div>
